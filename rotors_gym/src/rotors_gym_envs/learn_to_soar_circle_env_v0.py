...
class LearnToCircleEnv(gym.Env):

    def __init__(self):
        rospy.init_node('gym_circle', anonymous=True)

        # Time-step toggle:
        self.time_step = 0.3

        # Utils for action limits:
        self.pitch_cmd_limit = 1.0 #0.3 * np.pi
        self.roll_cmd_limit  = 1.0 #0.3 * np.pi
        self.yaw_cmd_limit   = 0.25 * np.pi

        # Utils for ground floor:
        ...
        self.yaw_pub   = rospy.Publisher("/l2s/attitude_cmd/yaw", Float32, queue_size=1)
        ...
        self.gained_energy      = None
        self.lost_energy        = None
    ...
    def reset(self):
        # theoretically limits altitude to 50 m if it doesn't gain energy
        E = 50 * 9.81 

        z_randn = 2 * (self.np_random.rand() - 0.5) # number in [-1,1]
        z = 7.5 + 7.5 * z_randn                     # initial alt in [0m, 15m]
        v = np.sqrt(2 * (E - 9.81 * z))

        yaw_randn = 2 * (self.np_random.rand() - 0.5)
        yaw = -0.0 + np.pi/2 * yaw_randn                # initial yaw in [0m, 15m]

        pitch_randn = 2 * (self.np_random.rand() - 0.5)
        pitch = 0.17 * np.pi * pitch_randn              # initial pitch in ~[-30°, 30°]

        roll_randn = 2 * (self.np_random.rand() - 0.5)
        roll =  0.17 * np.pi * roll_randn               # initial roll in ~[-30°, 30°]
        ...
        # Set initial position, orientation, speed:
        ...
        self.init_state.twist.linear.y  = v * np.sin(yaw) * np.cos(pitch) + self.get_wind(z)[1]
        self.init_state.twist.linear.z  = v * np.cos(pitch)
        ...
        self.last_energy = E
        self.gained_energy = 0.0
        self.lost_energy = 0.0
        ...
        self.yaw_pub.publish(Float32(self.yaw_cmd))
    ...
    def _observe(self, observation_only=False):
        ...
        deltaE = self.current_energy - self.last_energy
        self.last_energy = self.current_energy
        self.gained_energy  += max([0, deltaE])
        self.lost_energy    += abs(min([0, deltaE]))

        pos = 0.8
        reward = pos * self.gained_energy - (1 - pos) * self.lost_energy

        done = False

        if z < -8:
            done = True
            reward += min([-100, -(self.current_energy - 10)**2 - 100])
        ...
        if done and self.episode_start_time is not None: 
            ...
            now = rospy.Time.now()
            self.episode_duration = now.to_time() - self.episode_start_time.to_time()
            
            self.terminal_energy = self.current_energy
            self.gained_energy   = self.gained_energy
            self.lost_energy     = self.lost_energy
            self.positive_power  = self.gained_energy/self.episode_duration 

            reward += 10 * (self.gained_energy - self.lost_energy)
    ...
    def _apply_action(self, action):
        ...
        self.yaw_cmd  = action[2] * self.yaw_cmd_limit
